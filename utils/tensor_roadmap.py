...
class TensorRoadmap(object):
    ...
    *Vertices of a roadmap of a single robot are Point_2, while vertices of the
    ...
    :type roadmaps: dict<:class:`~discopygal.solvers.Robot`, :class:`networkx.Graph`>
    :param nearest_neighbors: a nearest neighbors algorithm. if None then use sklearn implementation
    :type nearest_neighbors: :class:`~discopygal.solvers.nearest_neighbors.NearestNeighbors` or :class:`None`
    ...
    def find_best_edge(self, tensor_vertex, tensor_point):
        ...
        vertex_2_coords = Point_2(vertex[2], vertex[3])

        # Compute distance
        dist_1 = self.metric.dist(tensor_vertex1_coords, vertex_1_coords).to_double()
        dist_2 = self.metric.dist(tensor_vertex2_coords, vertex_2_coords).to_double()
    ...
    def try_connecting(self, vertex1, vertex2):
        ...
        for i, robot_i in enumerate(self.robots):
            for j, robot_j in enumerate(self.robots):
                ...
                edge_i = Segment_2(vertex2_points[i], vertex2_points[i])

                # Robot j moves along its path
                for k in range(len(paths[j]) - 1):
                    edge_j = Segment_2(paths[j][k], paths[j][k + 1])
                ...
                edge_i = Segment_2(vertex1_points[i], vertex1_points[i])
